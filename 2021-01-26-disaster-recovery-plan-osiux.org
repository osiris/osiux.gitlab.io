...
Ayer fue mi primer deía de laburo del 2021 y arranqué mal, o mejor
...
parcial, la mejora ahi fue utilizar `nice -n 19 rsync -avP` e irse a
dormir.
...
remover el M2, ponerlo en un adaptador USB, conectarlo a otro equipo y
ponerse a copiar.

Gracias nuevamente a compañerxs de gcoop, en un par de horas ya
contaba con el adaptador M2 y pude verificar que al menos las
...
que se ocupa de sincronizar mis repositorios importates cada 5min
...
movil lo podes perder (yo me olvide ni notebook en un colectivo y tuve
la suerte de recuperarla) o con mala suerte lo pueden robar o consumirse
en un incendio, entonces la clave esta en poder recuperase rapidamente y
para ello es necesario un **plan de recuperacion de desastres** y lo
basico es contar con las herramientas (sistema operativo, aplicaciones y
utilitarios), la configuracion especifica y lo irremplazable, los datos.
...
que considero vitales para mi entorno en el repositorio
 [[https://gitlab.com/osiux/direplos][direplos]], si bien quedó desactualizado el
script =deb.sh= y a medio refactorizar y sin publicar la versión
ansible, pero con mínimos cambios pude ejecutar la instalacion con el comando:
...
Por ser previsor, gran parte de lo que necesito, esta (o deberia estar)
...
que trabajo o trabajé en algun momento, como asi tambien todo lo que me
...
administrarlos se alguns manera.
...
Lo bueno de personalizar tu entorno al maximo es que sos extremadamente
agil en cada tarea, por mas simple que sea y yo tengo muchisimo tiempo
invertido en optimizar mi entorno porque basicamente practicamente todo
lo hago dentro de una terminal.

Lo malo es que si no tenes tu entorno disponible, te volves muy torpe y
lento, por este motivo es vital contar con un procedimiento que permita
levantar tu entorno rapidamente en cualquier equipo y con un minimo
...
dificil y costoso fue no tener segmentado y priorizado, es decir, tener
...
el minimo entorno necesario y en dejar un test que a diario regenere mi
entorno a los efectos de encontrar el equilibrio entre un deploy rapido
y un entorno minimo que permita estar operativo ante un desastre o un
simple cambio de equipo en un par de minutos en lugar de un par de horas.
...
** ChangeLog
   ...
   - [2021-01-26 mar] Correcciones menores de formato y link roto
   - [2021-01-27 mie] Agrego explicación sobre direplos y haga intro a git-repos
   - [2021-01-28 jue] Completo seccion sobre entorno/eficiencia y corrijo formato
